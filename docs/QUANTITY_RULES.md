...
| bucket.rule.alt.variant | alternative | Another way to meet the need line bucket.rule (the food need's cost as freeze-dried meals, its days beyond the first month as bulk staples, part of the stored water as reused bottles). Never add it to the need. |
...
| `bleach_bottles` | water | water_boil (water_out when there is no boil target) | water_treatment_capacity | target(water_boil), target(water_out) | max(1, ceil(longer target ÷ 14)) bottles of plain unscented 5–9 % bleach; *Prior*: one bottle covers water treatment and cleaning for two weeks | bottle | household | cdc_water_storage, epa_emergency_disinfection, cdc_bleach_disinfecting, rr_expert_prior | implemented (requested by content) |
...
| `pet_food_days` | food | supplies | pet_food | dogs, cats, small pets, target(supplies) | (dogs + cats + small pets) × max(days, 7) pet-days (ASPCA 7–10 days; no per-pet calorie figure is published) | pet_day | pet | aspca_disaster_prep, ready_gov_pets | implemented (requested by content) |
...
| `sleeping_bag_or_blanket` | thermal | thermal (cold) | thermal_cold | people | 1 per person, rated for the coldest nights | item | person | ready_gov_kit, sphere_2018 | rr-supply |
| `warm_layers` | thermal | thermal (cold) | thermal_cold | people | 1 set of warm layers (coat, hat, gloves) per person | set | person | ready_gov_kit, cdc_winter_safety | rr-supply |
...
| `extinguisher_count` | fire | fire | fire_extinguisher | housing.kind, basement, alarms.extinguisher | 0 when the household has one; else one per level (apartment or mobile home 1, house 2, +1 basement); *Prior* level count | extinguisher | household | usfa_extinguishers, rr_expert_prior | implemented (requested by content); 0 when `alarms.extinguisher` is true |
| `escape_ladder_count` | fire | fire | escape_ladder | housing.kind, housing.floor | 1 for a house with bedrooms upstairs (detached, rowhouse, rural property) or an apartment on floor 2–3; else 0 (*Prior*) | ladder | household | ready_gov_home_fires, rr_expert_prior | implemented (requested by content) |
...
| `get_home_bag` | get home | get_home | get_home_bag | each person's `commute` (distance above 0), hot | 1 bag per commuter (a line per person): walking shoes, light, map, cash, weather layer, water and food for the walk (1,250 kcal per 12 h, *Prior*); kept in the car (car commuters) or at work; be ready to shelter at work 24 h | bag | commuter | ready_gov_kit_2020, ready_gov_kit, fhwa_mutcd_walking_speed, rr_expert_prior | rr-supply |
| `get_home_water` | get home | get_home | walking_water | commute distance, hot | hours = miles ÷ 3 mph (*Prior*; 2–2.4 loaded); litres = hours × 0.5 L an hour (*Prior*; 0.71, NIOSH, when hot); past 2.5 L carry a filter | litre | commuter | cdc_niosh_heat_hydration, fhwa_mutcd_walking_speed, rr_expert_prior | rr-supply |
...
| `go_bag` | evacuate | evacuate | go_bag | people aged 4+, target(evacuate) notice and days away | 1 bag per person aged 4 and over (babies' things go in a parent's bag); where it lives follows the notice band (under 1 h minutes, under 24 h hours, else days; *Prior* band edges) | bag | person | washington_prepare_in_a_year, ready_gov_kit, cdc_evacuation_psa, rr_expert_prior | rr-supply |
| `go_bag_water` | evacuate | evacuate | go_bag_water | people, water_level, hot, target(evacuate).days_away | people's per-day water × min(days away, 3) days (Red Cross three-day evacuation supply); pets have their own kit | gallon | person | redcross_survival_kit, washington_prepare_in_a_year, ready_gov_water | rr-supply |
| `go_bag_food` | evacuate | evacuate | go_bag_food | people, target(evacuate).days_away | DGA kcal a day × min(days away, 3); food that needs no cooking | kcal | person | redcross_survival_kit, usda_dga_2020_2025, washington_prepare_in_a_year | rr-supply |
...
| `pet_go_water` | evacuate | evacuate | pet_water | dogs, cats, small pets | pets' water per day × 7 days (ASPCA); replaced every 2 months | gallon | pet | aspca_disaster_prep, rr_research_supply_standards | rr-supply |
...
days when there is none); `bleach_bottles` uses the longer of **water_boil** and **water_out**,
`cooking_fuel_canisters` the longer of **supplies** and **water_boil**, and `toilet_paper_rolls` the
longer of **supplies** and **water_out**. Heat lines (`battery_fan`, `cooling_towel`,
`cooling_plan`) appear when `heat_wave` drives the thermal bucket, cold lines
(`sleeping_bag_or_blanket`, `warm_layers`, `warm_room_plan`) when `cold_wave`, `winter_weather` or
`ice_storm` does, and both when the assessment names neither.
...
Sources the registry does not carry (PetMD and the Merck Veterinary Manual for pet water, Germany's
and Denmark's water guides, the CDC Yellow Book heat chapter, the generator maker's fuel figures)
are cited through `rr_research_supply_standards`, the research report that records them.
