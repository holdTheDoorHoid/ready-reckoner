...
| `pet_food_lb` | supplies days, pets | (dogs × 0.7 + cats × 0.15 + small pets × 0.05) lb a day × max(days, 7). *Prior* feeding amounts for a 40-lb dog and a 10-lb cat (the sizes the water rule assumes); ASPCA advises 7–10 days of food | item unit | aspca_disaster_prep, ready_gov_pets, rr_expert_prior | requested by content |
...
| `generator_fuel_gallons` | power days, generator | 0 without a generator; else min(2.8 gal a day × days, 25 gal) gallons (a 2.2 kW inverter unit at quarter load; the 25-gallon cap is the fire-code limit, 10 gal in an attached garage) | gallon | rr_research_supply_standards, lehi_fuel_storage | requested by content |
