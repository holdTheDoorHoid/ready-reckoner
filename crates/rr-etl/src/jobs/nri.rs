...
const ITEM: &str = "https://www.arcgis.com/sharing/rest/content/items/39485e8035d446a5bff03259508ae355?f=json";
const ITEM_PAGE: &str = "https://fema.maps.arcgis.com/home/item.html?id=39485e8035d446a5bff03259508ae355";
const DICTIONARY: &str = "https://fema.maps.arcgis.com/sharing/rest/content/items/4b9db412e99542029b3c37c37ad714bb/data";
...
pub const SEMANTICS: &[Semantics] = &[
    Semantics { code: "AVLN", id: "avalanche", afreq_kind: "events_per_year", basis: "distinct_events", unit: "loss-causing avalanche events per year (county)", period: "30 years (1994-2023)", poisson_ok: true, section: "6.5", notes: "Counts from SHELDUS, the Colorado Avalanche Information Center and NOAA. Counties in an avalanche susceptibility zone with no recorded loss-causing event get a minimum of 0.01 per year." },
    Semantics { code: "CFLD", id: "coastal_flooding", afreq_kind: "events_per_year", basis: "modelled", unit: "modelled coastal flood occurrences per year, area-weighted over developed land", period: "not applicable (modelled)", poisson_ok: false, section: "7.4", notes: "Sum of sub-type frequencies: 0.01 inside the 1% annual-chance coastal floodplain, 0.002 inside the 0.2% floodplain, plus NOAA high-tide flooding classes (minor, moderate, major) at the midpoints of NOAA's probability classes. High-tide flooding recurs, so values above 1 are common and do not measure the chance of a damaging surge. Use the expected annual loss fields and flood.csv (share of homes in the flood zone) for household risk." },
    Semantics { code: "CWAV", id: "cold_wave", afreq_kind: "events_per_year", basis: "event_days", unit: "cold-wave days per year (NWS alerts), area-weighted", period: "19.0 years (2005-11-12 to 2024-11-12)", poisson_ok: false, section: "8.5", notes: "Table 5 of the documentation calls the measure an 'annualized probability', but the section text and the values (often above 1) are event-days per year. Days cluster into episodes: divide by a typical episode length before treating the result as episodes per year." },
    Semantics { code: "DRGT", id: "drought", afreq_kind: "events_per_year", basis: "event_days", unit: "drought days per year (US Drought Monitor event-weeks times 7), area-weighted", period: "25.25 years", poisson_ok: false, section: "9.5", notes: "Event-days, not events: a single drought can contribute hundreds of days. Divide by a typical drought length before treating it as droughts per year." },
    Semantics { code: "ERQK", id: "earthquake", afreq_kind: "annual_probability", basis: "modelled", unit: "annual probability of at least minor-damage shaking (USGS probability grid), area-weighted", period: "not applicable (modelled)", poisson_ok: false, section: "10.4", notes: "Already a yearly probability; do not transform. seismic.csv carries USGS exceedance probabilities at 0.1 g and 0.2 g at the county centroid for cross-checking." },
    Semantics { code: "HAIL", id: "hail", afreq_kind: "events_per_year", basis: "distinct_events", unit: "hail events per year (hail 0.75 in or larger; 49-km fishnet, scaled), area-weighted", period: "38.0 years (1986-2023)", poisson_ok: true, section: "11.5", notes: "Fishnet counts include hail that fell near, not only inside, the county; a regional frequency rather than the chance hail hits one home." },
    Semantics { code: "HWAV", id: "heat_wave", afreq_kind: "events_per_year", basis: "event_days", unit: "heat-wave days per year (NWS alerts), area-weighted", period: "19.0 years (2005-11-12 to 2024-11-12)", poisson_ok: false, section: "12.5", notes: "Event-days per year. Days cluster into episodes: divide by a typical episode length before treating the result as episodes per year." },
    Semantics { code: "HRCN", id: "hurricane", afreq_kind: "events_per_year", basis: "distinct_events", unit: "hurricane events per year (buffered HURDAT2 tracks on a 49-km fishnet)", period: "173.86 years for the Atlantic (1851-01-01 to 2024-09-30); 76.0 years for the Pacific (1949-01-01 to 2024-11-30)", poisson_ok: true, section: "13.5", notes: "Minimum frequencies are assigned to blocks in hurricane-prone counties with no recorded track. events.csv adds hurricane and tropical-storm passage rates within 50 nautical miles of the county centroid for cross-checking." },
    Semantics { code: "ISTM", id: "ice_storm", afreq_kind: "events_per_year", basis: "event_days", unit: "ice-storm days per year (49-km fishnet), area-weighted", period: "67.16 years (1946-12-31 to 2014-02-12)", poisson_ok: false, section: "14.5", notes: "Event-days per year; the record ends in 2014. events.csv has ice-storm episode rates from NOAA Storm Events 1996-2025." },
    Semantics { code: "IFLD", id: "riverine_flooding", afreq_kind: "events_per_year", basis: "event_days", unit: "inland flood days per year (NCEI Storm Events flood and flash-flood records, one per county-day)", period: "27 years (1996-2023, as stated)", poisson_ok: false, section: "15.5", notes: "New in v1.20: Inland Flooding replaced Riverine Flooding (RFLD) and includes rain-driven (pluvial) flooding. About 100% of a county's building value is treated as exposed, so EALB/EXPB is a county-wide average rather than an in-floodplain rate. Counties crossing the 1% floodplain with no recorded flood day get a minimum of 0.01. Pair with flood.csv (share of homes in the flood zone)." },
    Semantics { code: "LNDS", id: "landslide", afreq_kind: "events_per_year", basis: "modelled", unit: "landslide events per year (USGS county rate, weighted by the 2024 USGS susceptibility model)", period: "not applicable (modelled rate)", poisson_ok: true, section: "16.5", notes: "County value is the USGS estimated events per county per year; tracts inherit it in proportion to susceptibility." },
    Semantics { code: "LTNG", id: "lightning", afreq_kind: "events_per_year", basis: "event_days", unit: "days per year with a cloud-to-ground lightning strike (SPC daily lightning climatology), area-weighted", period: "29 years in the contiguous US (1995-2023); 12 years elsewhere (2012-2024)", poisson_ok: false, section: "17.5", notes: "Table 5 calls the measure an 'annualized probability', but values are days per year (often 20 to 80). No lightning data for Guam, the Northern Mariana Islands, American Samoa or the US Virgin Islands." },
    Semantics { code: "SWND", id: "strong_wind", afreq_kind: "events_per_year", basis: "distinct_events", unit: "strong-wind events per year (49-km fishnet, scaled), area-weighted", period: "38.0 years (as stated; the documentation's date range reads 1996-2023)", poisson_ok: true, section: "18.5", notes: "Fishnet counts include events near, not only inside, the county." },
    Semantics { code: "TRND", id: "tornado", afreq_kind: "events_per_year", basis: "distinct_events", unit: "tornado events per year (80-km buffered paths on a 49-km fishnet, nationally scaled by EF sub-type)", period: "74.0 years for EF2-EF5 (1950-2023); 38.0 years for EF0-EF1 (1986-2023)", poisson_ok: true, section: "19.5", notes: "A frequency of tornadoes in the county's surroundings, not the chance a tornado strikes one home; use EALB/EXPB or HLRB for damage. events.csv adds SPC tornado counts inside the county." },
    Semantics { code: "TSUN", id: "tsunami", afreq_kind: "annual_probability", basis: "modelled", unit: "surrogate yearly frequency: regional historical runup rate plus P-2426 probabilistic frequency", period: "222.15 years for the historical part", poisson_ok: false, section: "20.3", notes: "FEMA states this value is a surrogate 'strictly for reference' and is not used in NRI's own loss calculations. Use it only to flag tsunami exposure (nri_counties.csv tsunami_zone) and use the expected annual loss fields for magnitude." },
    Semantics { code: "VLCN", id: "volcanic_activity", afreq_kind: "annual_probability", basis: "modelled", unit: "yearly eruption frequency from USGS National Volcanic Threat Assessment recurrence scores (score 4 = 0.0101, 3 = 0.001, 2 = 0.0002)", period: "not applicable (recurrence classes)", poisson_ok: false, section: "21.4", notes: "Very small by construction; a per-volcano recurrence class, area-weighted." },
    Semantics { code: "WFIR", id: "wildfire", afreq_kind: "annual_probability", basis: "modelled", unit: "annual burn probability from a large fire (USFS FSim), area-weighted over developed or agricultural land", period: "not applicable (modelled)", poisson_ok: false, section: "22.4", notes: "Already a yearly probability that fire reaches a point; do not transform. Building/population frequency can differ from agriculture frequency." },
    Semantics { code: "WNTW", id: "winter_weather", afreq_kind: "events_per_year", basis: "event_days", unit: "winter-weather days per year (NWS alerts), area-weighted", period: "19.0 years (2005-11-12 to 2024-11-12)", poisson_ok: false, section: "23.5", notes: "Event-days per year. Days cluster into storms: divide by a typical storm length before treating the result as storms per year." },
];
...
pub fn extract_disclaimer(license_html: &str) -> Option<String> {
    ...
    let sentence = after[..close].split_whitespace().collect::<Vec<_>>().join(" ");
    if sentence.len() < 40 { None } else { Some(sentence) }
}
...
pub fn run(ctx: &Ctx) -> Result<JobOutput> {
    ...
    for s in SEMANTICS {
        rr_types::HazardId::from_str(s.id).map_err(|_| data_err(format!("semantics table uses unknown hazard id {}", s.id)))?;
    }
    ...
    let dict = ctx.http.get(DICTIONARY, Some("nri/NRIDataDictionary.csv"))?;
    let (dh, drows) = crate::csvout::parse_delimited(&dict.text(), b',')?;
    let (di_field, di_ver, di_date) =
        (crate::csvout::col(&dh, "Field Name")?, crate::csvout::col(&dh, "Version")?, crate::csvout::col(&dh, "Version Date")?);
    let version = drows.first().map(|r| r[di_ver].clone()).unwrap_or_default();
    let version_date = drows.first().map(|r| r[di_date].clone()).unwrap_or_default();
    ...
    for f in &fields {
        if !dict_fields.contains(f) {
            return Err(data_err(format!("NRI data dictionary no longer lists field {f}")));
        }
    }
    ...
    if absent.len() > 12 {
        return Err(data_err(format!("NRI data dictionary lacks {} expected hazard fields (e.g. {:?}); the layout changed", absent.len(), &absent[..5])));
    }
    if !absent.is_empty() {
    ...
    loop {
        ...
        let feats = v["features"].as_array().ok_or_else(|| data_err("NRI query: no features array"))?;
        let n = feats.len();
        for f in feats {
    ...
    out.source(super::source_from("NRI item metadata and Terms & Conditions", &item, format!("item modified {}", meta["modified"]), "Terms & Conditions", ""));
    out.source(super::source_from("NRI data dictionary", &dict, format!("{version} ({version_date})"), "Terms & Conditions", ""));
    ...
    let mut ht = Table::with_header(
        ["fips", "hazard"].iter().map(|s| s.to_string()).chain(HAZARD_FIELDS.iter().map(|(_, c)| c.to_string())).collect(),
        2,
    );
    ...
    let mut coastal_n = 0;
    let mut tsunami_n = 0;
    let get = |r: &serde_json::Map<String, serde_json::Value>, k: &str| r.get(k).and_then(|v| v.as_f64()).filter(|v| v.is_finite());
    for r in &records {
        let fips = r.get("STCOFIPS").and_then(|v| v.as_str()).unwrap_or_default().to_string();
        ...
        let tsunami = get(r, "TSUN_EXPB").is_some_and(|v| v > 0.0) || get(r, "TSUN_EXPP").is_some_and(|v| v > 0.0);
        ...
        ct.push(row);
        for s in SEMANTICS {
            let vals: Vec<Option<f64>> = HAZARD_FIELDS.iter().map(|(suf, _)| get(r, &format!("{}_{}", s.code, suf))).collect();
    ...
    if !extra.is_empty() {
        out.notes.push(format!("NRI records not in the canonical county list were skipped: {}.", extra.join(", ")));
    }
    ...
    out.missing = missing_groups(&counties, &covered, |_| "No National Risk Index record for this county".to_string());
    ...
    out.definitions.insert("nri_version".into(), format!("{version} ({version_date})"));
    out.definitions.insert("nri_citation".into(), citation.clone());
    out.definitions.insert("nri_disclaimer".into(), disclaimer.clone());
...
fn semantics_toml(version: &str, version_date: &str, nri_ver: &str) -> String {
    ...
    s.push_str("# clients, so the document was read from the Internet Archive snapshot named below.\n");
    ...
    s.push_str(&format!("technical_documentation = {}\n", toml_str(TECH_DOC)));
    s.push_str(&format!("technical_documentation_archive = {}\n", toml_str(TECH_DOC_ARCHIVE)));
    for h in SEMANTICS {
        s.push_str("\n[[hazard]]\n");
