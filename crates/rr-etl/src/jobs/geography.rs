...
const GAZ_COUNTIES: &str = "https://www2.census.gov/geo/docs/maps-data/data/gazetteer/2024_Gazetteer/2024_Gaz_counties_national.zip";
const GAZ_ZCTA: &str = "https://www2.census.gov/geo/docs/maps-data/data/gazetteer/2024_Gazetteer/2024_Gaz_zcta_national.zip";
const REL_ZCTA_COUNTY: &str = "https://www2.census.gov/geo/docs/maps-data/data/rel2020/zcta520/tab20_zcta520_county20_natl.txt";
const CT_TOWNS: &str =
    "https://www2.census.gov/geo/docs/reference/ct_change/ct_cou_to_cousub_crosswalk.txt";
const CT_ZCTA_TOWN: &str =
...
const EXCLUDED: &[(&str, &str)] = &[
    ...
    (
        "69085",
        "Northern Islands, Northern Mariana Islands (no NRI record)",
    ),
];
...
pub fn run(ctx: &Ctx) -> Result<JobOutput> {
    ...
    let cb500 = ctx
        .http
        .get(CB500, Some("geography/cb_2024_us_county_500k.zip"))?;
    out.source(source_from(
        "Census cartographic boundary counties 2024, 1:500,000",
        &cb500,
        version_of(&cb500, "GENZ2024"),
        PUBLIC_DOMAIN,
        "",
    ));
    let excluded: BTreeSet<&str> = EXCLUDED.iter().map(|(f, _)| *f).collect();
    let mut cb: Vec<CbCounty> = read_cb(&cb500.bytes)?
        .into_iter()
        .filter(|c| !excluded.contains(c.fips.as_str()))
        .collect();
    cb.sort_by(|a, b| a.fips.cmp(&b.fips));
    out.rows_in += cb.len() as u64;

    let gaz = ctx.http.get(
        GAZ_COUNTIES,
        Some("geography/2024_Gaz_counties_national.zip"),
    )?;
    out.source(source_from(
        "Census 2024 Gazetteer, counties",
        &gaz,
        version_of(&gaz, "2024 Gazetteer"),
        PUBLIC_DOMAIN,
        "",
    ));
    let gtext = String::from_utf8_lossy(&zip_entry(&gaz.bytes, ".txt")?).to_string();
    let (gh, grows) = parse_delimited(&gtext, b'\t')?;
    let (g_id, g_lat, g_lon, g_land) = (
        col(&gh, "GEOID")?,
        col(&gh, "INTPTLAT")?,
        col(&gh, "INTPTLONG")?,
        col(&gh, "ALAND_SQMI")?,
    ...
    for r in &grows {
        let (Ok(lat), Ok(lon)) = (r[g_lat].parse::<f64>(), r[g_lon].parse::<f64>()) else {
            continue;
        };
        gaz_pts.insert(
            r[g_id].clone(),
            (lat, lon, r[g_land].parse().unwrap_or(f64::NAN)),
        );
    }
    ...
    let regions_doc = ctx
        .http
        .get(NCA_REGIONS, Some("geography/nca_regions.json"))?;
    out.source(source_from(
    ...
    for f in rj["features"]
        .as_array()
        .ok_or_else(|| data_err("NCA regions: no features"))?
    {
        let name = f["attributes"]["RegionName"].as_str().unwrap_or_default();
        let id = region_id(name)
            .ok_or_else(|| data_err(format!("NCA regions: unknown region name {name:?}")))?;
    ...
    for c in &cb {
        if let Some(p) = gaz_pts.get(&c.fips) {
            ...
            let cen = c
                .poly
                .centroid()
                .ok_or_else(|| data_err(format!("no centroid for {}", c.fips)))?;
            points.insert(
                c.fips.clone(),
                (cen[1], cen[0], c.aland_m2 / 2_589_988.110336),
            );
    ...
    for c in &cb {
        ...
        let voted = votes.get(&c.state_abbr).and_then(|v| {
            v.iter()
        ...
        let region = match voted {
            ...
            None => {
                ...
                fallback_region(&c.state_abbr)
                    .ok_or_else(|| data_err(format!("no NCA region for {}", c.state_abbr)))?
            }
        };
        state_region.insert(c.state_abbr.clone(), region);
    }

    let mut counties = Table::new(
        &[
            "fips",
            "name",
            "name_full",
    ...
    let ct_towns = ctx
        .http
        .get(CT_TOWNS, Some("geography/ct_cou_to_cousub_crosswalk.txt"))?;
    out.source(source_from(
        "Census Connecticut county-to-county-subdivision crosswalk",
        &ct_towns,
        version_of(&ct_towns, "ct_change crosswalk"),
        PUBLIC_DOMAIN,
        "",
    ));
    let ct_zcta = ctx.http.get(
        CT_ZCTA_TOWN,
        Some("geography/acs22_cousub22_zcta520_st09.txt"),
    )?;
    out.source(source_from(
        "Census 2022 Connecticut county subdivision to 2020 ZCTA relationship file",
        &ct_zcta,
        version_of(&ct_zcta, "rel2022 acs22"),
        PUBLIC_DOMAIN,
        "",
    ));
    let (th, trows) = parse_delimited(&ct_towns.text(), b'|')?;
    let (t_state, t_old, t_new, t_geoid) = (
        col_prefix(&th, "STATEFP")?,
        col_prefix(&th, "OLD_COUNTYFP")?,
        col_prefix(&th, "NEW_COUNTYFP")?,
        col_prefix(&th, "NEW_COUSUB_GEOID")?,
    );
    ...
    let rel = ctx.http.get(
        REL_ZCTA_COUNTY,
        Some("geography/tab20_zcta520_county20_natl.txt"),
    )?;
    out.source(source_from(
        "Census 2020 ZCTA to county relationship file",
        &rel,
        version_of(&rel, "rel2020"),
        PUBLIC_DOMAIN,
        "",
    ));
    ...
    for r in &rrows {
        ...
        out.rows_in += 1;
        let (zl, zw) = (
            r[r_zland].parse::<f64>().unwrap_or(0.0),
            r[r_zwater].parse::<f64>().unwrap_or(0.0),
        );
        zip_total.insert(r[r_z].clone(), (zl, zw));
        let (pl, pw) = (
            r[r_land].parse::<f64>().unwrap_or(0.0),
            r[r_water].parse::<f64>().unwrap_or(0.0),
        );
    ...
    for r in &zrows {
        ...
        *ct_new_land.entry(r[z_zcta].clone()).or_default() += pl;
        ...
        zip_total
            .entry(r[z_zcta].clone())
            .or_insert((r[z_zland].parse().unwrap_or(0.0), 0.0));
    }
        ...
        for (c, s) in shares {
            ...
            if !canon.contains(&c) {
                return Err(data_err(format!(
                    "ZIP {z} points at county {c}, which is not in the 2024 county list"
                )));
            }
    ...
    let cb20 = ctx
        .http
        .get(CB20, Some("geography/cb_2024_us_county_20m.zip"))?;
    out.source(source_from(
        "Census cartographic boundary counties 2024, 1:20,000,000",
        &cb20,
        version_of(&cb20, "GENZ2024"),
        PUBLIC_DOMAIN,
        "",
    ));
    ...
    let not_on_map: Vec<String> = canon
        .iter()
        .filter(|c| !map_ids.contains(*c))
        .cloned()
        .collect();
    out.notes.push(format!(
    ...
    for c in map {
        ...
        let ring_str = |r: &Vec<[f64; 2]>| {
            let pts: Vec<String> = r
                .iter()
                .map(|p| format!("[{},{}]", fixed(p[0], 3), fixed(p[1], 3)))
                .collect();
            format!("[{}]", pts.join(","))
        };
        let poly_str = |p: &Vec<Vec<[f64; 2]>>| {
            format!("[{}]", p.iter().map(ring_str).collect::<Vec<_>>().join(","))
        };
        if polys.len() == 1 {
            s.push_str(&format!(
