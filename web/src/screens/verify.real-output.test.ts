...
function root(): string {
  ...
  while (!existsSync(join(dir, "fixtures", "golden"))) {
    ...
    if (up === dir)
      throw new Error("fixtures/golden not found above the working directory");
    dir = up;
  }
  return dir;
}
...
function golden(name: string): PlanOutput {
  return JSON.parse(
    readFileSync(join(root(), "fixtures", "golden", `${name}.json`), "utf8"),
  ) as PlanOutput;
}
...
describe("screens with the real engine output (verification)", () => {
  ...
  it("the real plan can list a deposit and its purchase for one item in the same month", () => {
    ...
    // Since v0.1.1 other sinking funds (smoke alarms, the get-home bags) complete earlier than the
    // cash reserve, so look for the cash reserve's own month rather than the first duplicate key.
    const month = out.plan.months.find(
      (m) =>
        m.items.filter((i) => i.item_id === "docs_cash_reserve").length > 1,
    );
    expect(
      month,
      "a month with a reserve and a purchase of the cash reserve",
    ).toBeDefined();
    const keys = month!.items.map((i) => `${i.item_id}${i.tier}`);
    expect(
      new Set(keys).size,
      "the month repeats an item id + tier key",
    ).not.toBe(keys.length);
    const kinds = month!.items
      .filter((i) => i.item_id === "docs_cash_reserve")
      .map((i) => i.kind);
    expect(kinds.sort()).toEqual(["purchase", "reserve"]);
  });

  it("the risks screen renders the real Philadelphia output", async () => {
    const r = await render(Risks, {
      plan: savedFor(FIXTURES[name]),
      route: "risks",
  ...
  it("V-10 (fixed by keyedItems): the plan screen renders the real Philadelphia plan", async () => {
    const r = await render(PlanScreen, {
      plan: savedFor(FIXTURES[name]),
      route: "plan",
      engine: answering(golden(name)),
    });
